...
export function CourseDetailClient({ initialCourse, courseId }: CourseDetailClientProps) {
  ...
  // Use the initial course data from SSR
  const [course] = useState(initialCourse)
  
  // State for enrollment dialog
  const [showEnrollDialog, setShowEnrollDialog] = useState(false)
  
  // Use the student course slice for enrollment status
  const { 
    enrolledCourses,
    loadEnrolledCourses
  } = useAppStore()
              ...
              <div className="lg:col-span-2">
                ...
                <div className="mb-6 flex flex-wrap items-center gap-6 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{course.rating_average || course.rating || 4.5}</span>
                    <span className="text-muted-foreground">
                      ({course.rating_count || Math.floor((course.enrollment_count || 0) * 0.8)} ratings)
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Users className="h-4 w-4" />
                    <span>{(course.enrollment_count || course.enrollmentCount || 0).toLocaleString()} students</span>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>{course.language === 'en' ? 'English' : course.language || 'English'}</span>
                  </div>
                </div>
